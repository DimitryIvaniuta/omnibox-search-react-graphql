/* eslint-disable */
import * as types from './graphql';
import { TypedDocumentNode as DocumentNode } from '@graphql-typed-document-node/core';

/**
 * Map of all GraphQL operations in the project.
 *
 * This map has several performance disadvantages:
 * 1. It is not tree-shakeable, so it will include all operations in the project.
 * 2. It is not minifiable, so the string of a GraphQL query will be multiple times inside the bundle.
 * 3. It does not support dead code elimination, so it will add unused operations.
 *
 * Therefore it is highly recommended to use the babel or swc plugin for production.
 * Learn more about it here: https://the-guild.dev/graphql/codegen/plugins/presets/preset-client#reducing-bundle-size
 */
type Documents = {
    "fragment SearchHitContactFields on SearchHitContact {\n  id\n  score\n  subtitle\n  title\n  contactId\n}\n\nfragment SearchHitListingFields on SearchHitListing {\n  id\n  score\n  subtitle\n  title\n  listingId\n  mlsId\n}\n\nfragment SearchHitMailingFields on SearchHitMailing {\n  id\n  score\n  subtitle\n  title\n  mailingId\n}\n\nfragment SearchHitProductFields on SearchHitProduct {\n  id\n  score\n  subtitle\n  title\n  productId\n}\n\nfragment SearchHitReferralFields on SearchHitReferral {\n  id\n  score\n  subtitle\n  title\n  referralId\n}\n\nfragment SearchHitTransactionFields on SearchHitTransaction {\n  id\n  score\n  subtitle\n  title\n  transactionId\n}": typeof types.SearchHitContactFieldsFragmentDoc,
    "query Omnibox($q: String!, $limitPerGroup: Int = 5) {\n  omnibox(q: $q, limitPerGroup: $limitPerGroup) {\n    contacts {\n      ...SearchHitContactFields\n    }\n    listings {\n      ...SearchHitListingFields\n    }\n    mailings {\n      ...SearchHitMailingFields\n    }\n    products {\n      ...SearchHitProductFields\n    }\n    referrals {\n      ...SearchHitReferralFields\n    }\n    transactions {\n      ...SearchHitTransactionFields\n    }\n  }\n}": typeof types.OmniboxDocument,
};
const documents: Documents = {
    "fragment SearchHitContactFields on SearchHitContact {\n  id\n  score\n  subtitle\n  title\n  contactId\n}\n\nfragment SearchHitListingFields on SearchHitListing {\n  id\n  score\n  subtitle\n  title\n  listingId\n  mlsId\n}\n\nfragment SearchHitMailingFields on SearchHitMailing {\n  id\n  score\n  subtitle\n  title\n  mailingId\n}\n\nfragment SearchHitProductFields on SearchHitProduct {\n  id\n  score\n  subtitle\n  title\n  productId\n}\n\nfragment SearchHitReferralFields on SearchHitReferral {\n  id\n  score\n  subtitle\n  title\n  referralId\n}\n\nfragment SearchHitTransactionFields on SearchHitTransaction {\n  id\n  score\n  subtitle\n  title\n  transactionId\n}": types.SearchHitContactFieldsFragmentDoc,
    "query Omnibox($q: String!, $limitPerGroup: Int = 5) {\n  omnibox(q: $q, limitPerGroup: $limitPerGroup) {\n    contacts {\n      ...SearchHitContactFields\n    }\n    listings {\n      ...SearchHitListingFields\n    }\n    mailings {\n      ...SearchHitMailingFields\n    }\n    products {\n      ...SearchHitProductFields\n    }\n    referrals {\n      ...SearchHitReferralFields\n    }\n    transactions {\n      ...SearchHitTransactionFields\n    }\n  }\n}": types.OmniboxDocument,
};

/**
 * The graphql function is used to parse GraphQL queries into a document that can be used by GraphQL clients.
 *
 *
 * @example
 * ```ts
 * const query = graphql(`query GetUser($id: ID!) { user(id: $id) { name } }`);
 * ```
 *
 * The query argument is unknown!
 * Please regenerate the types.
 */
export function graphql(source: string): unknown;

/**
 * The graphql function is used to parse GraphQL queries into a document that can be used by GraphQL clients.
 */
export function graphql(source: "fragment SearchHitContactFields on SearchHitContact {\n  id\n  score\n  subtitle\n  title\n  contactId\n}\n\nfragment SearchHitListingFields on SearchHitListing {\n  id\n  score\n  subtitle\n  title\n  listingId\n  mlsId\n}\n\nfragment SearchHitMailingFields on SearchHitMailing {\n  id\n  score\n  subtitle\n  title\n  mailingId\n}\n\nfragment SearchHitProductFields on SearchHitProduct {\n  id\n  score\n  subtitle\n  title\n  productId\n}\n\nfragment SearchHitReferralFields on SearchHitReferral {\n  id\n  score\n  subtitle\n  title\n  referralId\n}\n\nfragment SearchHitTransactionFields on SearchHitTransaction {\n  id\n  score\n  subtitle\n  title\n  transactionId\n}"): (typeof documents)["fragment SearchHitContactFields on SearchHitContact {\n  id\n  score\n  subtitle\n  title\n  contactId\n}\n\nfragment SearchHitListingFields on SearchHitListing {\n  id\n  score\n  subtitle\n  title\n  listingId\n  mlsId\n}\n\nfragment SearchHitMailingFields on SearchHitMailing {\n  id\n  score\n  subtitle\n  title\n  mailingId\n}\n\nfragment SearchHitProductFields on SearchHitProduct {\n  id\n  score\n  subtitle\n  title\n  productId\n}\n\nfragment SearchHitReferralFields on SearchHitReferral {\n  id\n  score\n  subtitle\n  title\n  referralId\n}\n\nfragment SearchHitTransactionFields on SearchHitTransaction {\n  id\n  score\n  subtitle\n  title\n  transactionId\n}"];
/**
 * The graphql function is used to parse GraphQL queries into a document that can be used by GraphQL clients.
 */
export function graphql(source: "query Omnibox($q: String!, $limitPerGroup: Int = 5) {\n  omnibox(q: $q, limitPerGroup: $limitPerGroup) {\n    contacts {\n      ...SearchHitContactFields\n    }\n    listings {\n      ...SearchHitListingFields\n    }\n    mailings {\n      ...SearchHitMailingFields\n    }\n    products {\n      ...SearchHitProductFields\n    }\n    referrals {\n      ...SearchHitReferralFields\n    }\n    transactions {\n      ...SearchHitTransactionFields\n    }\n  }\n}"): (typeof documents)["query Omnibox($q: String!, $limitPerGroup: Int = 5) {\n  omnibox(q: $q, limitPerGroup: $limitPerGroup) {\n    contacts {\n      ...SearchHitContactFields\n    }\n    listings {\n      ...SearchHitListingFields\n    }\n    mailings {\n      ...SearchHitMailingFields\n    }\n    products {\n      ...SearchHitProductFields\n    }\n    referrals {\n      ...SearchHitReferralFields\n    }\n    transactions {\n      ...SearchHitTransactionFields\n    }\n  }\n}"];

export function graphql(source: string) {
  return (documents as any)[source] ?? {};
}

export type DocumentType<TDocumentNode extends DocumentNode<any, any>> = TDocumentNode extends DocumentNode<  infer TType,  any>  ? TType  : never;